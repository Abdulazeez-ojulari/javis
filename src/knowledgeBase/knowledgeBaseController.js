...
const {
  ...
  getKnowledgeBaseFromFileService,
  createKnowledgeBaseFromCsvService,
...
module.exports.createKnowledgeBase = errorMiddleware(async (req, res) => {
  ...
  if (knowledge) {
    return res
      .status(400)
      .send({ message: "A knowledge base already exists for this business" });
  }
  ...
  let newKnowledgeBase = await createKnowledgeBaseService(
    businessId,
    knowledgeBase,
    faqs
  );
...
module.exports.updateKnowledgeBase = errorMiddleware(async (req, res) => {
  let { userId, businessId, knowledgeBase, type, faqs, companyInformation } =
    req.body;
  ...
  let newKnowledgeBase = await updateKnowledgeBaseService(
    businessId,
    knowledgeBase,
    faqs,
    companyInformation
